...
const HomePage = () => {
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      const listRef = ref(storage, 'Cantos'); // Reference to the folder
  ...
  const handleFileClick = (url) => {
    setPreviewUrl(url); // Set the preview URL when a file card is clicked
  };

  return (
    <div className={styles.container}>
      <h1>Cantos Infantiles</h1>
      <div className={styles.cardContainer}>
        {files.map((file) => (
          <div 
            className={styles.card} 
            key={file.name} 
            onClick={() => handleFileClick(file.url)} // Handle click to show preview
          >
            <h2>{file.name}</h2>
